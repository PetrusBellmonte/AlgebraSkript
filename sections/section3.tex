...
\begin{theorem}
    Das $n$ste Kreisteilungspolynom $\phi_n$ ist irreduzibel in $\Q[X]$, d.h.
...
\end{definition}
Für jede Menge $M$ ist die Menge $Abb(M,K)$ der Abbildungen von $M$ nach $K$ ein $K$-Vektorraum bez punktweiser Addition und skalarer Multiplikation.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        Angenommen $\sum_{i=1}^n c_i \chi_i = 0$, wobei nicht alle $c_i$ Null sind.
        Sei $\mu\in \Gamma$ mit $\chi_1(\mu) \neq \chi_2(\mu)$.
        Dann gilt für alle $\gamma \in \Gamma$:
        \begin{align*}
            (1):& 0 = \sum_{i=1}^n c_i\chi_i(\mu\gamma) = \sum_{i=1} ^n c_i \chi_i(\mu) \chi_i(\gamma)\\
            (2):& 0 = \chi_1(\mu)\sum_{i=1}^n c_i \chi_i(\gamma) = \sum_{i=1}^n c_i \chi_1(\mu)\chi_i(\gamma)\\
            (1) - (2):& 0= \sum_{i=2}^n c_i (\underbrace{\chi_i(\mu) - \chi_1(\mu)}_{\neq 0 für i=2}) \cdot \chi_i(\gamma)
        \end{align*}
        \Lightning zu I-Annahme
    \end{itemize}
\end{proof}
\begin{corollary}
...
\end{corollary}
\begin{proof}
    Sei $\sigma_1,\dots,\sigma_n\in Aut(K)$. Dann wende 3.17 auf ${\sigma_1}_{|K^\times},\dots, {\sigma_n}_{|K^\times}$ an.
\end{proof}
\begin{lemma}
...
\begin{proof}$ $
    \begin{itemize}
        \item[(ii)=>(i)] Sei $\sum_{j=0}^n \lambda_i v_i = 0$, wobei nicht alle $\lambda_j$ Null sind.
        Dann ist $\sum_{j=0}^n \lambda_j \sigma_i(v_j) = 0$ für alle i.
        D.h. die Spalte von $(\sigma_i(v_j))_{ij}$ sind linear abhängig.
        \item[(i)=>(ii)] Sei $\det = 0$. Dann gibt es $\mu_1,\dots,\mu_n\in \overline{K}$ mit $\sum_{i=1}^n\mu_i \sigma_i(v_j) = 0$ für alle $j$ (Zeilen lin. abh).
        Nach 3.17 sind ${\sigma_1}_{|L^\times},\dots,{\sigma_n}_{|L^\times}$ linear unabhängig.
...
\begin{theorem}[Satz von der Normalbasis]
    ...
    Es existiert ein $a\in L$, sodass $\sigma_1(a),\dots,\sigma_n(a)$ eine $K$-Basis von $L$ bilden.
\end{theorem}
\end{document}