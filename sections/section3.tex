...
\begin{document}
\begin{definition}
    Eine algebraische Körpererweiterung $L|K$ heißt \imph{Galoiserweiterung} (oder galois'sch), wenn $L|K$ normal und separabel ist.
    Man nennt dann $Gal(L|K) = Aut_K(L)$ die \imph{Galoisgruppe} von $L|K$.

    Sei $F$ ein Körper und $H\leq Aut(F)$ eine Untergruppe.
    dann ist $F^H = \{x\in F\mid \forall\sigma\in H:\sigma(x) = x\}$ ein Teilkörper von F, genannt \imph{Fixkörper} von H.
\end{definition}
...
\begin{itemize}[align= left]
    ...
    \begin{itemize}
        ...
        %$E=L^{Gal(L|E)} \overset{\sigma\Gamma(\cdot)\sigma^{-1}}{\Longrightarrow} \sigma Gal(L|E) \sigma^{-1} = \sigma \Gamma(E) \sigma^{-1} = Gal(L|E)$ 
...
\begin{definition}
    ...
    Ein Element $\xi\in K^\times$ mit $\xi^n=1$ heißt \imph{$n$-te Einheitswulzel (EW)}.
    $\mu_n(K)\coloneqq\{\text{n-te EW in K}\}\leq K^\times$ zyklische Untergruppe.
    $\card{\mu_n(K)}\leq n$\\
    Hat $\xi$ die Ordnung $n$, so nennt man $\xi$ \imph{primitive $n$-te EW}.
...
\begin{definition}
    Der Körper $\Q(\xi_n)$ für $\xi_n\in\mu_n^*(\C)$ heißt $n$-ter \imph{Kreisteilungskörper}.
    Das Polynom $\phi_n=\prod_{\xi\in\mu_n^*(\C)} (X-\xi)$ heißt $n$-tes \imph{Kreisteilungspolynom}
\end{definition}
...
\begin{definition}
    ...
    Ein \imph{Charakter} (von $\Gamma$ nach $K$) ist ein Homomorphismus $\Gamma\rightarrow K^\times$.
\end{definition}
...
\begin{theorem}[Satz von der Normalbasis]
    ...
    Eine solche Basis nennen wir \imph{Normalbasis} von $L|K$.
\end{theorem}
...
\begin{definition}$ $
    \begin{enumerate}[label=(\roman*)]
        \item man sagt eine $K$-Erweiterung $L|K$ sei \imph{durch Radikale auflösbar}, wenn ein Turm von endlichen $K$-Erweiterungen gibt $K=K_0\subseteq K_1\subseteq K_2\dots\subseteq K_r$, sodass
        \begin{itemize}
...
\begin{definition}\label{theo:3.28}
    Sei $L|K$ endlich.
    Sei $a\in L$. Die \imph{Spur} $Sp_{L|K}(a)\in K$ ist die Spur des $K$-lin. Endomorphismus $\phi_a:L\rightarrow L, \phi_a(x) = a\cdot x$.
    Ähnlich definiert man die \imph{Norm} $N_{L|K}(a) \in K$ als $\det(\phi_a)$.
...
\begin{proof}
    ...
    \end{pmatrix}$$
    $\Rightarrow$ $Sp_{L|K}(a) = -\alpha_{n-1}\quad$%TODO Format that shit
