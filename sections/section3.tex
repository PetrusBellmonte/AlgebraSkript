...
\begin{proof}$ $
    \begin{itemize}
        ...
        Falls $<\{v_1,\dots,v_n\}> = L$, dann gilt $\sum_{i=1}^n \mu_i\sigma_i = 0$ im Widerspruch zur linearen Unabhängigkeit.
...
\TODO[Formatting...]
\begin{proof}
    (für unendliche Körper $K$):
    Gemäß 3.19 reicht es ein Element $a\in L$ zu finden, mit $\det(\sigma_i(\sigma_j(a)))_{ij}\neq 0$.
    Nach dem Satz vom primitiven Element gibt es ein $b\in L$ mit $K(b)=L$.
    Dann sind $b_i\coloneqq\sigma_i(b)$ die paarweise verschiedenen Nullstellen von $f\coloneqq m_{b,K}=\Pi_{i=1}^n (X-\sigma_i(b))\in K[X]$.
    Es ist $b_1=b$.
    Setze $$g_j = \Pi_{i\neq j}\frac{X-b_i}{b_j-b_i}\in L[X]$$
    Dann gilt $g_j(b_k) = \delta_{jk} = \begin{cases}
        1 & \text{, wenn} j=k\\
        0 & \text{, wenn} j\neq k
    \end{cases}$.
    Weiter ist 
    $$(1)\quad g_1+\dots+g_n=1$$, weil die linke Seite Grad $\leq n-1$ hat und beide Seiten für $b_1,\dots,b_n$ übereinstimmen.
    $$(2)\quad \sigma_{j*}(g_1) = \sigma_{j*}(\Pi_{i\neq j}\frac{X-b_i}{b_1-b_i}) = \Pi_{i\neq j}\frac{X-\sigma_{j*}(b_i)}{b_j-\sigma_{j*}(b_i)}=\Pi_{i\neq j}\frac{X-b_i}{b_j-b_i} = g_j$$
    Betrachte die Matrix $$A=(\sigma_{i*}(g_j))_{ij} \in M_n(L[X])$$
    Beh: $\det(A) \neq 0\in L[X]$
    \begin{itemize}
        \item $(g_i\cdot g_j)(b_k) = \delta_{ik}*\delta_{jk} = 0$ falls $i\neq j$.
        => $f= m_{b,K}|(g_i\cdot g_j)$ für $i\neq j$. (smile)
        \item Multipliziere $(1)$ mit $g_i$. Dann ergibt sich, dass $g_i^2 = g_i \mod f\cdot L[X]$.
        (weil wenn du (1) mit $g_i$ multipliziert und dann alles außer $g_i^2=g_i$ wegfällt).
    \end{itemize}
    Sei $B=AA^T = (\beta_{ij})_{ij} \in M_n(L[X])$.
    Dann gilt $\beta_{ij} = \sum_{k=1}^n\sigma_{ik}(g_k)\sigma_{jk} (g_k) \overset{(2)}{=} \sum_{k=1}^n (\underbrace{\sigma_i\circ \sigma_k}_{= \sigma_{m(i,k)}})_* (g_1) \cdot (\underbrace{\sigma_j\circ\sigma_k}_{\sigma_{m(j,k)}})_* (g_1)$
    $i=j$: $\beta_{ii} =\sum_{k=1}^n g_k^2\overset{(*)}{=} \sum_{k=1}^n g_k\mod f\cdot L[X] \overset{(1)}{=} 1$
    $i\neq j$: $m(i,k) \neq m(j,k)$ für alle $k$.
    Wegen (smile) also $\beta_{ij}\equiv 0\mod f\cdot L[X]$.

    => $B\equiv I_n\mod f\cdot L[X]$
    => $\det(A)^2 = \det(B)\equiv 1 \mod f\cdot L[X]$.
    ...
    Dann folgt:
    \begin{align*}
        0\neq p(u) &= \det\left((\sigma_{i*}(g_j)(u))_{ij}\right)\\
        &= \det\left(((\sigma_i\circ \sigma_j)_* g_j(u))_{ij}\right)\\
        &= \det\left(((\sigma_i\circ \sigma_j)(a))_{ij}\right)\\
        &= \det\left(((\sigma_i)(\sigma_j(a)))_{ij}\right)
    \end{align*}
\end{proof}
...
\end{definition}
Die Nullstellen von $X^n-a$ in $\overline{K}$ sind $$\{\xi\cdot \sqrt[n]{a}\mid \xi \mu_n(\overline{k})\}$$.
Der Zerfällungskörper ist $K(\xi,\sqrt[n]{a})$, wobei $\xi$ eine primitive $n$-te EW ist.

\begin{theorem}
    Sei $K$ ein Körper, $n\in \N$ mit $char(K)\not\div n$ (0 teilt keine zahl!!).
...
\begin{proof}
    \begin{itemize}
        ...
        * $\psi: Gal(K(\sqrt[n]{a})|;)\rightarrow \mu_n(K), \sigma \mapsto \omega_\sigma = \frac{\sigma(\sqrt[n]{a})}{\sqrt[n]{a}}$ ist eine injektiver Gruppenhomomorphismus:
        $\psi(\sigma\tau) = \frac{\sigma(\tau(\sqrt[n]{a}))}{\sqrt[n]{a}} = \frac{\sigma(\omega_\tau \cdot \sqrt[n]{a})}{\sqrt[n]{a}} = \omega_\tau \cdot \frac{\sigma(\sqrt[n]{a})}{\sqrt[n]{a}}
        = \omega_\tau \cdot \omega_\sigma = \psi(\tau)\psi(\sigma)$
        * injektiv, weil $\sigma$ durch $\omega_\sigma$ eindeutig feistgelegt wird, da $\sqrt[n]{a}$ prim Element von $K(\sqrt[n]{a})$.
        ...
        Also $m_{b,K} = \Pi_{i=0}^{n-1} (X-\xi^i\cdot b) = X^n-b^n\in K[X]$.
        Wähle $a=b^n$.
    \end{itemize}
\end{proof}
