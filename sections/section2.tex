...
\begin{definition}[Inhalt]
    Sei A ein Hauptidealring und $K = Quot(A)$. Sei $f = \sum_{0 \leq i \leq n} a_i X^i \in A[X]$. Definiere $\Tilde{c}(f) \in A$ als einen Erzeuger des Ideals $(a_0, ..., a_n) \subset A$ (eindeutig bis auf Multiplikation mit Einheiten/inv. Elemente in A). Die Assoziiertenklasse $c(f) = \Tilde{c}(f)A^\times \subset A/A^\times$ [$A^\times$ invertierbare Elemente in A] heißt \imph{Inhalt} von f.\\
...
\begin{definition}[Endliche Erweiterung]
    Sei $L | K$ eine Erweiterung. Dann ist L insb. ein $K-VR$. Die Dimension über K von L $dim_K(L) =: [L:K]$ heißt der \imph{Grad der Körpererweiterung} $L | K$. Die Erweiterung heißt \emph{endlich}\index{Endliche Körpererweiterung}, wenn $[L:K] < \infty$.
\end{definition}
...
\begin{definition}[Adjungieren]
    Sei $L | K$ eine Körpererweiterung. Sei $S \subset L$ eine Teilmenge. Dann bezeichnet $K(S)$ den kleinsten Teilkörper von L, der K und S enthält. Für $S=\{\alpha\}$ schreibt man $K(\alpha) = K(\{\alpha\})$ (gesprochen "K adjungiert $\alpha$"). Man nennt eine Körpererweiterung $K(\alpha) | K$ \imph{einfach}.
\end{definition}
\begin{definition}[Algebraisch vs. Transzendent]
    ...
    Falls $ev_\alpha$ injektiv ist, so nennt man $\alpha$ \imph{transzendent} (dann ist $f(\alpha) \equiv 0 \Rightarrow f \equiv 0$, also ist $\alpha$ nicht Nullstelle eines nichttrivialen Polynoms). Es gilt $Bild(ev_\alpha) \cong K[X]$ und $K(X) \cong K(\alpha)$. Insb. $[K(\alpha):K] = \infty$.
    
    Falls dagegen $ev_\alpha$ nicht injektiv ist, nennt man $\alpha$ \imph{algebraisch}. Dann ist $ker(ev_\alpha) = (m_{\alpha, K})$ Hauptideal. O.B.d.A. sei $m_{\alpha, K}$ normiert (Leitkoeffizient = 1). Wir nennen $m_{\alpha, K}$ das \imph{Minimalpolynom} von $\alpha$ über K. Dann $L \supset Bild(ev_\alpha) \cong K[X]/(m_{\alpha, K})$ nullteilerfrei. Folglich ist $m_{\alpha, K}$ irreduzibel. Damit ist $(m_{\alpha, K})$ sogar ein maximales Ideal in K[X], also ist $Bild(ev_\alpha) \cong K[X]/(m_{\alpha, K})$ sogar ein Körper, also $Bild(ev_\alpha) = K(\alpha)$. (Schreibe auch $K[\alpha] := Bild(ev_\alpha)$.) Es ist $[K(\alpha):K] = deg \; m_{\alpha, K} < \infty$.
\end{definition}
\begin{example}
...
\end{proof}
\begin{remark}
Sei $L \mid K$ eine K-Erweiterung. Die Menge $L_{alg, K} := \{\alpha \in L \mid \alpha \text{ algebraisch über K}\}$ ist ein Zwischenkörper $L-L_{alg, K}-K$. Diesen nennt man den \imph{algebraischen Abschluss} von K in L.
...
\end{proof}
\begin{definition}
    Ein Körper $K$ ist \imph{algebraisch abgeschlossen}, falls jedes nicht-konstante Polynom $f \in K[X]$ eine Nullstelle in $K$ besitzt.
\end{definition}
...
\end{proof}
\begin{definition}
    Sei K ein Körper. Es gibt einen algebraisch abgeschlossenen Körper $\overline{K}$ so, dass $K \subseteq \overline{K}$ und $\overline{K}\setminus K$ algebraisch ist. Man nennt $\overline{K}$ einen \imph{algebraischen Abschluss}. 
\end{definition}
\begin{proof}
...
\begin{definition}
    Seien $L_1| K$ und $L_2|K$ $K$-Erweiterungen. Ein Homomorphismus $f: L_1 \rightarrow L_2$ heißt \imph{$K$-Homomorphismus}, falls $f(k) = k$ für alle $k \in K$. Ein K-Isomorphismus ist ein bijektiver K-Homomorphismus. Definiere ${Aut}(L_1|K) = \{f: L_1 \rightarrow L_1 \mid f \; K-Isom.\}$ (Gruppe der K-Automorphismen mit Verknüpfung als Operation).
\end{definition}
...
\begin{definition}
    Sei $K$ ein Körper und $\Omega \subseteq K[X]$ eine Teilmenge von nicht konstanten Polynomen. Ein Erweiterungskörper $L$ von $K$ heißt \imph{Zerfällungskörper} von $\Omega$, falls gilt
    \begin{enumerate}
        \item Jedes $f\in \Omega$ zerfällt in $L[X]$ in Linearfaktoren
        \item $L=K(S)$, wobei $S=\{x\in L \mid \exists f\in \Omega: f(x)=0\}$
    \end{enumerate}
    Eine Körpererweiterung $L|K$ heißt \imph{normal}, falls sie ein Zerfällungskörper für eine Menge $\Omega \subseteq K[X]\setminus K$ ist.
\end{definition}
...
\begin{definition}
    Ein irreduzibles Polynom in $K[X]$ heißt \imph{separabel}, wenn es im $\overline{K}$ nur einfache Nullstellen hat. (allgemein heißt ein Polynom separabel, wenn alle irreduzieblen Faktoren separabel sind)
\end{definition}

\begin{definition}
    Die $K$-lineare Abbildung $D:K[X]\rightarrow K[X], \sum a_ix^i \mapsto \sum ia_iX^{i-1}$ heißt \imph{(formal) Ableitung}.
...
\begin{definition}
    ...
    Der \imph{Separabilitätsgrad} $[L:K]_S$ über $K$ ist definiert als $\card{Hom_K(L,\overline{K})}$.
...
\end{proof}
Ein Körper $K$ ist \imph{perfekt}, wenn jede alg. Erweiterung von $K$ separabel ist.
...
\end{proof}
\begin{theorem}
    $Aut(\F_{p^n}| \F_p)$ ist zyklisch von Ordung $n$. Sie wird erzeugt vom \imph{Frobenius-Automorphismus}:
    \begin{align*}
