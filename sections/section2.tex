...
\begin{definition}[Inhalt]
    Sei A ein Hauptidealring und $K = Quot(A)$. Sei $f = \sum_{0 \leq i \leq n} a_i X^i \in A[X]$. Definiere $\Tilde{c}(f) \in A$ als einen Erzeuger des Ideals $(a_0, ..., a_n) \subset A$ (eindeutig bis auf Multiplikation mit Einheiten/inv. Elemente in A). Die Assoziiertenklasse $c(f) = \Tilde{c}(f)A^* \subset A/A^*$ [$A^*$ invertierbare Elemente in A] heißt \emph{Inhalt} von f.\\
    Sei $f \in K[X]$. Wähle $a \in A \setminus \{0\}$ mit $af \in A[X]$. Dann definiere $\Tilde{c}(f) = c(af)a^{-1} \in K$ und $c(f) = \Tilde{c}(f)A^* \in K/A^*$ (Übung: Def. unabh. von der Wahl von a).
\end{definition}
\begin{example*}
...
\begin{proof}
    ...
    $$h := \Tilde{c}(g_0)\Tilde{c}(h_0)\Tilde{c}(h_0)^{-1}h_0 \overset{\text{\ref{theo:2.5} b}}{=} \underbrace{\Tilde{c}(\overbrace{g_0h_0}^{=f})}_{\in A^\times}a\ \underbrace{\Tilde{c}(h_0)^{-1}h_0}_{\in A[X]}$$ für geeignetes $a \in A^*$. Somit ist auch $h \in A[X]$. Weiter ist $f = gh$. Dann ist $\deg g_0 = \deg g = \deg f$ oder $\deg h_0 = \deg h = \deg f$.
\end{proof}
...
\begin{proof}$ $\\
    ...
    \begin{minipage}{0.8\textwidth}
        ...
        Da $\overline{\sigma_i},\overline{\sigma_s}$ automatisch inj. sind\footnotemark, ist $\tau_j = \tau_r$ also $j = r$.
