...
\begin{definition}[Wiederholung aus EAZ]
    Sei $K$ im Folgenden ein Körper. Der Polynomring $K[X]$ ist ein Hauptidealring\footnote{nullteilerfrei, Kommultativ und jedes Ideal ist Hauptideal}. Das Ideal $(f)$ ist Primideal gdw. $f = 0$ oder $f$ irreduzibles Polynom ist, d.h. $f = gh \Rightarrow g \in K^* \vee h \in K^*$.
\end{definition}
...
\begin{definition}[Endliche Erweiterung]
    Sei $L | K$ eine Erweiterung. Dann ist L insb. ein $K-VR$. Die Dimension über K von L $dim_K(K) =: [L:K]$ heißt der Grad der Körpererweiterung $L | K$. Die Erweiterung heißt endlich, wenn $[L:K] < \infty$.
\end{definition}
...
\begin{definition}[Adjungieren]
    Sei $L | K$ eine Körpererweiterung. Sei $S \subset L$ eine Teilmenge. Dann bezeichnet $K(S)$ den kleinsten Teilkörper von L, der K und S enthält. Für $S=\{\alpha\}$ schreibt man $K(\alpha) = K(\{\alpha\})$ (gesprochen "K adjungiert $\alpha$"). Man nennt eine Körpererweiterung $K(\alpha) | K$ einfach.
\end{definition}
\begin{definition}[Algebraisch vs. Transzendent]
    ...
    Falls $ev_\alpha$ injektiv ist, so nennt man $\alpha$ transzendent (dann ist $f(\alpha) \equiv 0 \Rightarrow f \equiv 0$, also ist $\alpha$ nicht Nullstelle eines nichttrivialen Polynoms). Es gilt $Bild(ev_\alpha) \cong K[X]$ und $K(X) \cong K(\alpha)$. Insb. $[K(\alpha):K] = \infty$.
    
    Falls dagegen $ev_\alpha$ nicht injektiv ist, nennt man $\alpha$ algebraisch. Dann ist $ker(ev_\alpha) = (m_{\alpha, K})$ Hauptideal. O.B.d.A. sei $m_{\alpha, K}$ normiert (Leitkoeffizient = 1). Wir nennen $m_{\alpha, K}$ das Minimalpolynom von $\alpha$ über K. Dann $L \supset Bild(ev_\alpha) \cong K[X]/(m_{\alpha, K})$ nullteilerfrei. Folglich ist $m_{\alpha, K}$ irreduzibel. Damit ist $(m_{\alpha, K})$ sogar ein maximales Ideal in K[X], also ist $Bild(ev_\alpha) \cong K[X]/(m_{\alpha, K})$ sogar ein Körper, also $Bild(ev_\alpha) = K(\alpha)$. (Schreibe auch $K[\alpha] := Bild(ev_\alpha)$.) Es ist $[K(\alpha):K] = deg \; m_{\alpha, K} < \infty$.
\end{definition}
\begin{example}
...
\begin{theorem}
    \begin{enumerate}
        \item Ist $L \mid K$ endlich, adnn ist $L \mid K$ algebraisch.
        \item Ist $L \mid K$ algebraisch und endlich erzeugt (d.h. $L = K(\alpha_1, ..., \alpha_n)$ für geeignete $\alpha_i$), dann ist $L \mid K$ endlich.
        \item Sind $L \mid K$ und $M \mid L$ algebraisch, so ist auch $M \mid K$ algebraisch.
    \end{enumerate}
\end{theorem}
\begin{proof}
    \begin{itemize}
...
\end{proof}
\begin{remark}
Sei $L \mid K$ eine K-Erweiterung. Die Menge $L_{alg, K} := \{\alpha \in L \mid \alpha \text{ algebraisch über K}\}$ ist ein Zwischenkörper $L-L_{alg, K}-K$. Diesen nennt man den algebraischen Abschluss von K in L.
...
\begin{proof}
    ...
    $$f(\alpha) = f(\pi(X)) = \sum_{i=0}^n c_i\pi(X)^i = \pi(\sum_{i=0}^n c_iX^i) = \pi(f) = 0 \in L$$
    Also $[L:K] = [K(\alpha):K] = deg(f)$, weil f Minimalpolynom von $\alpha$ ist.
\end{proof}
\begin{corollary}\label{theo:2.20}
Seien K ein Körper und $f \in K[X]$ ein nicht-konstantes Polynom (irreduzible Polynome sind per Definition nicht-konstant). Es gibt eine endliche K-Erweiterung $L \mid K$, so dass f über L in Linearfaktoren zerfällt.
\end{corollary}
\begin{proof}
    Induktion über $deg(f) =: d$:
    \begin{itemize}
        \item[IA] $d = 1$: Klar, da f selbst Linearfaktor ist.
        \item[ISchritt] Sei $g \in K[X]$ ein irreduzibles Polynom, das f teilt. Nach dem vorhergehenden Satz gibt es $L_1 \mid K$, in der g eine Nullstelle $\alpha \in L_1$ besitzt. Schreibe $f = (X-\alpha)\Tilde{f}$ mit $\Tilde{f} \in L_1[X]$. Nach I-Annahme existiert $L \mid L_1$ endlich, in der $\Tilde{f}$ in Linearfaktoren zerfällt. Weiter $[L:K] = [L:L_1][L_1:L] < \infty$.
    \end{itemize}
\end{proof}
\begin{definition}
    Ein Körper K ist algebraisch abgeschlossen, falls jedes nicht-konstante Polynom $f \in K[X]$ eine Nullstelle in K besitzt.
\end{definition}
...
\end{theorem}
Auswahlaxiom in Algebra: nur hier, FAna: überall/Hahn-Banach, LA: Existenz von Basen unendlich dim VR
\begin{proof}
...
\end{proof}
\begin{definition}
    Sei K ein Körper. Es gibt einen algebraisch abgeschlossenen Körper $\overline{K}$ so, dass $K \subseteq \overline{K}$ und $\overline{K}\setminus K$\todo{oder $\overline{K}|K$? Zumindest steht das in anderer Aufzeichnung} algebraisch ist. Man nennt $\overline{K}$ einen algebraischen Abschluss. 
\end{definition}
\begin{proof}
    Sei $L \mid K$ eine alg. abgeschlossene Erweiterung. Sei $\overline{K} = L_{alg, K} = \{\alpha \in L \mid \alpha \text{ algebraisch über K}\}$. Z.z. $\overline{K}$ ist alg. abgeschlossen. Sei dafür $f \in \overline{K}[X]\setminus\overline{K}$. Es gibt eine Nullstelle $\alpha \in L$ von f. Dann ist $\alpha$ algebraisch über $\overline{K}$. Da $\overline{K}$ alg. über K, ist $\alpha$ alg. über K, also $\alpha \in \overline{K}$.
\end{proof}

\subsection{$\overline{K}$-Homomorphismen}
\begin{definition}
    Seien $L_1 \mid K$ und $L_2 \mid K$ K-Erweiterungen. Ein Homomorphismus $f: L_1 \rightarrow L_2$ heißt K-Homomorphismus, falls $f(k) = k$ für alle $k \in K$. Ein K-Isomorphismus ist ein bijektiver K-Homomorphismus. Definiere $Aut(L_1 \mid K) = \{f: L_1 \rightarrow L_2 \mid f \; K-Isom.\}$ (Gruppe der K-Automorphismen mit Verknüpfung als Operation).
\end{definition}
...
\begin{lemma} \label{theo:2.28}
    Seien $K$, $K'$ zwei Körper und $\sigma: K \rightarrow K'$ ein Isomorphismus. Sei $K(\alpha) \mid K$ eine einfache algebraische $K$-Erweiterung. Sei $L' \mid K'$ eine K'-Erweiterung. Für jede Nst. $\alpha' \in L'$ von $\sigma_*(m_{\alpha, K}) \in K'[X]$ ($\sigma_*$ wendet $\sigma$ auf die Koeffizienten an) gibt es genau einen Homomorphismus $\phi: K(\alpha) \rightarrow L'$ mit $\phi_{|K} = \sigma$ und $\phi(\alpha) = \alpha'$. Dann ist $\phi$ Isomorphismus zwischen $K(\alpha)$ und $K'(\alpha')$.
\end{lemma}
\begin{proof}
    Kommutatives Diagramm
\end{proof}
\begin{remark}
    Die Anzahl der Homom. $\phi$ wie im vorherigen Lemma ist genau die Anzahl der Nst. von $\sigma_*(m_{\alpha, K})$ in $L'$.
\end{remark}
\begin{example}
...
\begin{theorem}[Fortsetzungssatz]
    \begin{enumerate}[label=(\alph*)]
        \item Sei $L \mid K$ eine alg. K-Erweiterung, M ein alg. abgeschlossener Körper und $\sigma: K \rightarrow M$ ein Homom. Dann ex. $\phi: L \rightarrow M$ mit $\phi_{|K} = \sigma$.
...
\begin{proof}
    ...
    \TODO[Abbildung]
    Wegen Bemerkung \ref{theo:2.22} und der Algebraizität von $\overline{K'}|\phi(\overline{K})$ folgt $\phi(\overline{K}) = \overline{K'}$.
\end{proof}
\begin{flushright}
VL vom 13.11.2023:
\end{flushright}
...
\begin{definition}
    Sei $K$ ein Körper und $\Omega \subseteq K[X]$ ene Teilmenge von nicht konstanten Polynomen. Ein Erweiterungskörper $L$ von $K$ heißt \emph{Zeifällungskörper} von $\Omega$, falls gilt
    \begin{enumerate}
...
\begin{remark}
    ...
    Es gilt $[L:K]< m^r$.
    Tatsächlich gilt $[L:;] \leq m!$ (Übung).
    Natürlich \TODO[graphik] und $[K(\alpha_1,\dots,\alpha_i):K(\alpha_1,\dots,\alpha_{i-1})] \leq [K(\alpha_i):K] \leq m$
\end{remark}

\begin{example}
    \begin{enumerate}[label=\alph*)]
        \item $f=X^4 -2 \in \Q[X]$ irreduzibel nach Eisensteinkriterium mit $p=2$ und Nullstellen $\sqrt[4]{2},-\sqrt[4]{2},i\sqrt[4]{2}$,$-i\sqrt[4]{2}$ \footnote{\TODO Der mir vorliegende aufschieb hat hir nochmal die NS $\sqrt[4]{2}$. Ich vermute, dass ist ein fehler und ich habe es hier ersetzt}
        \begin{align*}
        ...
        \begin{proof}
            Angenommen $f=gh$: 
            Wenn $deg(g)=1$, hat $f$ eine Nullstelle in $\mathbb{F}_5$, aber $X^n \equiv 1 \mod{5}$ nach Satz von Euler\footnote{Satz von Euler: $X^{p-1} \equiv 1 \mod{p}$}.
            Wenn $deg(g)= 2 = deg(f)$ \TODO[was soll das für eine begründung sein?]
        \end{proof}
        Sei $\alpha\in \mathbb{F}_5$ eine Nullstelle von $f$. $\overline{\mathbb{F}}_5$ algebraischer Abschuss von $\mathbb{F}_5$.
        Dann ist $E = \mathbb{F}_5(\alpha) \cong \mathbb{F}_5[X]/(f)$ ein Zerfällungskörper von $f$, weil:
        $f$ hat die Nullstellen $\alpha, 2\alpha, 3\alpha, 4\alpha \in E$, da $b^4\equiv 1 \mod{5}$nach Euler für $b=2,3,4$.
...
\begin{theorem}[Charackterisung von normalen Erweiterungen]
    ...
    \begin{enumerate}
        ...
        \item Ledes irreduzible $f\in K[X]$, dass in $L$ eine Nullstelle besitzt, zerfällt in ´$L[X]$ in Linearfaktoren
    \end{enumerate}
\end{theorem}
\begin{proof}
    \TODO
...
\begin{proof}
    \TODO
\end{proof}
Ich habe mal die sätze der letzten vorlesung ergänzt
...
\begin{definition}
    Die $K$-lineare Abbildung $D:K[X]\rightarrow K[X], \sum a_ix^i \mapsto \sum ia_iX^{i-1}$ heizt \emph{(formal) Ableitung}.
...
\begin{example}
    $K = \F_p(T) = Quot(\F_p[T])$ (rationaler Funktiononkörper)
...
\begin{proof}[Beweis zu 2.4.0]
    ...
    "=>" Sei $D(f)=0$. Dann $D(f)(a_1)=0$. Dann $\exists i\neq1: a_i=a_1$, was \Lightning sep.

    "<=" Sei $D(f)\neq 0$.
    Wegen $\deg(D(f)) < \deg(f)$ und $f$ irreduzibel, sind $D(f)$ und $f$ teilerfremd.
    Es gibt also $g,k\in K[X]$ mit $1=gf+hD(f)$.

    Sei $a_k$ eine der Nullstellen in $\overline{K}$. Dann ist
    $$1=\equalto{g(a_k) f(a_k)}{0} + \equalto{h(a_k) D(f)(a_k)}{\neq 0}$$
...
\begin{lemma}
    \begin{enumerate}[label=\alph*)]
        \item Sind $L|K$ und $M|L$ alg. Erweiterungen, dann ist $[M:K]_S = [M:L]_S \cdot [L:K]_S$ (\TODO[M und L ist hier in ungewöhnlicher Reihenfolge. Korregieren??])
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Dann ist auch $L$ endlich. Nach \todo{2.55} ist $L^*$ zyklisch, d.h. $L^*=<a>$. 
        Somit $L=K(a)$.
...
\end{proof}
\begin{example}
    Die Separabilität im Satz \todo{2.56} ist essenziell.
