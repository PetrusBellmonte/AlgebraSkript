...
\begin{definition} % \gaf Generic absolut function
    Ein \imph{Betrag} auf einem Körper $K$ ist eine $\gaf: K\rightarrow \R_{\geq0}$ mit den Eigenschaften
...
\begin{example}$ $
    \begin{enumerate}[label=\roman*)] %i ii
        ...
        $$|x|_{trivial} = \begin{cases}
            ...
            0 & \text{falls }x= 0
        \end{cases}$$
        ein Betrag (genannt: \imph{trivialer Betrag})
...
\begin{definition} [p-adischer Betrag]
    ...
    der \imph{p-adische Betrag}.
Beachte $\nu_p(0) = \infty$.
\end{definition}
Damit gilt:
$$a = \pm \prod_{p\text{ prim}} p^{\nu_p(a)}$$
Die Funktion $\nu_p:\Z\rightarrow \N_0\cup \{\infty\}$ heißt \imph{p-adische Bewertung}. Sie hat die folgenden Eigenschaften
\begin{enumerate}[label=\roman*)]%i, ii
...
Dann definiert $\gaf_p$ einen Betrag auf $\Q$, den \imph{p-adischen Betrag}\footnote{Zum 2ten mal}. Die Eigenschaften i), ii) sind offensichtlich.
Zu iii):
\begin{align*}
    |x+y|_p &= p^{-\nu_p(x+y)}\\
    &\leq p^{-\min\{\nu_p(x),\nu_p(y)\}} \\
...
\end{align*}
%Multiline???
Dies wird \imph{ultrametrische Dreiecksungleichung} genannt.
\begin{definition}
...
\end{remark}
\begin{definition}
    Zwei Bewertungen $\gaf_1$, $\gaf_2$ sind \emph{äquivalent}\index{Aquivalent@Äquivalent!Bewertung}, wenn sie diesselbe Topologie auf $K$ induzieren.
\end{definition}
\begin{lemma}
...
\begin{definition}
    Ein Betrag $\gaf$ auf $K$ heißt \imph{archimedisch}, wenn 
...
\begin{definition}
    Ein Körper mit Betrag ist \imph{vollständig}, wenn jede Cauchyfolge\footnote{Eine Folge $(a_n)_{n\in\N}$ ist eine \imph{Cauchy-Folge}, wenn $\forall\epsilon>0\;\exists N\in \N\;\forall m,n\geq N\colon |a_m - a_n| <\epsilon$} in $K$ konvergiert.
\end{definition}
...
\begin{definition}% K^ statt tilde
    Sei $(K,\gaf)$ ein Körper mit Betrag. Eine \imph{Vervollständigung} von $(K,\gaf)$ ist ein Tripel $(\hat{K},\ggaf,i)$ bestehend aus:
    \begin{itemize}
...
\begin{definition}
    Die Vervollständigung von $\Q$ bezüglich des p-adischen Betrags $\gaf_p$ nennt man den \imph{Körper der $p$-adischen Zahlen} $\Q_p$
\end{definition}
...
\begin{theorem}\label{theo:4.23}
    ...
    Dann ist $$\mathcal{O}_\nu = \{x\in K\mid \nu(x)\geq0\}$$ [$\mathcal{O}_\nu = \{x\in K\mid \|x\|_\nu \leq 1\}$ für $\|x\|_\nu = a^{-\nu(x)}$] ein Ring (genannt \imph{Bewertungsring}).
...
\begin{definition}
    Ein kommutiver Ring $R$ mit genau einem maximalen Ideal $\mathcal{m}$ heißt \imph{lokaler Ring}. Der Körper $R/\mathcal{m}$ ist der \imph{Restklassenkörper} vorn $R$.
\end{definition}
...
\begin{definition}
    Man nennt eine Bewertung \imph{diskret}, wenn sie Werte in $\Z\cup \{\infty\}$ annimmt.
    Ein Element $\pi$ mit Bewertung $\nu(\pi) = 1$ heißt \imph{uniformisierndes Element}.
\end{definition}
...
\begin{theorem}%Satz und Definition
    ...
    \end{enumerate}
    Jeder Ring, der diese Bedingungen erfüllt, heißt \imph{diskreter Bewertungsring}.
\end{theorem}
\begin{proof} $ $
    \begin{itemize}
        \item[(a) $\Rightarrow$ (b)]
...
\begin{remark}[Die $p$-adischen Zahlen]
    ...
    Der Bewertungsring $\Z_p\coloneqq \mathcal{O}_{\nu_p}$ heißt der \imph{Ring der ganzen $p$-adischen Zahlen}.
