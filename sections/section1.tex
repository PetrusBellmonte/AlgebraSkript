...
\begin{reminder}
    Ein \emph{Normalteiler} $N$ einer Gruppe $(G,*)$ ist eine Untergruppe mit der Eigenschaft  $\forall g\in G: gNg^-1 = N$. Man defintiert auf den Nebenklassen $G/N = \{gN\mid g\in G\}$ eine Verknüpfung $g_1N \dot g_2N = g_1g_2N$, die aus $G/N$ eine Gruppe macht. Weiter ist $G\rightarrow G/N, g\mapsto gN$ ein Homomorphismus. Notation $N\nteq G$.

    Z.B. Die alternatierende Grruppe $A_n$ ist ein Normalteiler der symmethrischen Gruppe $S_3$.
...
Man sagt, dass $\mathcal{G}$ und $\mathcal{H}$ äquivalent sind, winn sie die gleiche Länge haben und es eine Permutation $\sigma \in S_n$ gibt mit $H_{i+1}/H_i \cong G_{\sigma(i)+1}/G_{\sigma(i)}$.
...
\begin{example}[Kompositionsreihen von $(G := \mathbb{Z},+)$]
...
\begin{theorem}
    Sind $\mathcal{G}$, $\mathcal{H}$ Normalreihen vorn $G$, dann gibt es Verfeinerung $\Tilde{\mathcal{G}}$, $\Tilde{\mathcal{H}}$ von $\mathcal{G}$, $\mathcal{H}$, sodass sie äquivalent sind.
\end{theorem}
\begin{proof}
    \TODO
\end{proof}
...
\end{proof}
\begin{remark*}
    Gleiche Kompositionsreihen $\centernot$ gleiche Gruppe.
\end{remark*}
\begin{example*}
    \begin{itemize}
...
\begin{example}
    ...
    \end{pmatrix} \mapsto (a,b)$$ Dann ist $N = ker(\phi) = \{\begin{pmatrix}
        1 & c \\ 0 & 1
    \end{pmatrix} \mid c \in \mathbb{K}\} \cong (\mathbb{K},+)$ Somit is $$\{id\} \triangleleftneq N \triangleleftneq B$$ eine Normalreihe mit abelschen Faktoren. (Geht auch für $GL_n(K)$, Beweis komplizierter)

    c) (Semidirektes Produkt von Gruppen) \TODO
\end{example}
\begin{theorem}[Untergruppen und Faktorgruppen auflösbarer Gruppen]\label{thm:ug:aufloesbar}
