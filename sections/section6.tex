...
\begin{definition}
    ...
    $R$ heißt \imph{ganze Hülle} von $A$ in $B$.
    Ist $R=B$, so sagt man, dass $B$ eine \imph{ganze Ringerweiterung} von $A$ ist.
    Ist $R=A$, so sagt man, dass $A$ \imph{ganz abgeschlossen} in $B$ ist.
    Ein Integritätsbereich, der ganz abgeschlossen in seinem Quotientenkörper ist, heißt \imph{ganz abgeschlossen}.
\end{definition}
\begin{lemma}
...
\begin{example*}
    \begin{enumerate}[label=(\arabic*)]
        ...
        $\mathcal{O}_K$ ist ein Beispiel eines \imph{Dedikindrings}, für die man Zahlentheorethische Betrachtungen analog zu Hauptidealringin durchführen.
    \end{enumerate}
\end{example*}
